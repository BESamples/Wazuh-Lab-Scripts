...
EOF
  ...
  echo "[+] YARA Active Response snippet ready: $YARA_AR_SNIPPET"
}
...
apply_yara_active_response() {
  ...
  echo "[+] Applying YARA Windows Active Response config..."
  create_yara_ar_snippet

  backup_file "$ACTIVE_OSSEC" "ossec.conf"

  # Remove the previous managed YARA block if this script already added it.
  sudo awk '
  ...
  # Add the managed YARA Active Response block before the closing ossec_config tag.
  sudo awk -v snippet="$YARA_AR_SNIPPET" '
    ...
    { print }
  ' "$ACTIVE_OSSEC.tmp" | sudo tee "$ACTIVE_OSSEC.tmp2" >/dev/null

  sudo mv "$ACTIVE_OSSEC.tmp2" "$ACTIVE_OSSEC"
  sudo rm -f "$ACTIVE_OSSEC.tmp"

  echo "[+] YARA Active Response config applied."
}
